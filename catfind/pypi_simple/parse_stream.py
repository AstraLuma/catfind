...
def parse_links_stream_response(
    r: httpx.Response, chunk_size: int = 65535
) -> Iterator[Link]:
    """
    .. versionadded:: 0.7.0

    Parse an HTML page from a streaming `httpx.Response` object and yield
    each hyperlink encountered in the document as a `Link` object.

    See `parse_links_stream()` for more information.

    :param httpx.Response r: the streaming response object to parse
